...
from ....models.entities import (
    Memory, MemoryType, ContentType, Priority, Status,
    Vector
)
...
class MemoryRepository(BaseRepository[Memory]):
    ...
    async def get_by_priority(
        ...
        try:
            if include_higher:
                ...
                valid_priorities = [
                    p.value for p, v in priority_order.items() 
    ...
    async def batch_update_access_tracking(
        self, 
        memory_ids: List[str]
    ) -> int:
        """
    ...
    async def get_frequently_accessed_memories(
        ...
        try:
            ...
            results = await self.db.execute_query(
                query, 
                (project_id, min_access_count, limit)
            )
            return [self.from_dict(row) for row in results]
