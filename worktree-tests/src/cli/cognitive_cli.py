...
@click.option('--format', '-f', type=click.Choice(['text', 'json', 'vtt']), default='text', help='Input format')
@click.option('--batch-size', '-b', default=32, help='Processing batch size')
@click.option('--output-format', '-o', type=click.Choice(['json', 'csv', 'summary']), default='summary', help='Output format')
@click.pass_context
async def ingest(ctx, transcript_path, meeting_title, meeting_id, format, batch_size, output_format):
    """
...
@click.option('--output-format', '-o', type=click.Choice(['json', 'table', 'detailed']), default='table', help='Output format')
...
async def query(ctx, query, max_results, include_bridges, output_format, explain):
    ...
    try:
        ...
        await _display_query_results(
            activated_memories, bridge_memories, 
            output_format, explain, max_results
        )
...
@click.option('--output-format', '-o', type=click.Choice(['json', 'table', 'summary']), default='summary', help='Output format')
@click.pass_context
async def status(ctx, include_performance, include_distribution, include_health, output_format):
    ...
    try:
        ...
        await _display_status(
            basic_status, performance_metrics, 
            distribution_data, health_data, output_format
        )
