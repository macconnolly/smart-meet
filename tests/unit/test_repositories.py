...
async def clear_db(db_connection):
    ...
    async with db_connection.get_connection() as conn:
            ...
            cursor = await loop.run_in_executor(db_connection._executor, lambda: conn.execute("SELECT name FROM sqlite_master WHERE type='table';"))
            tables = [row["name"] for row in await loop.run_in_executor(db_connection._executor, cursor.fetchall) if row["name"] != "sqlite_sequence"]
            for table in tables:
                await loop.run_in_executor(db_connection._executor, lambda t=table: conn.execute(f"DELETE FROM {t};"))
            await loop.run_in_executor(db_connection._executor, conn.commit)
