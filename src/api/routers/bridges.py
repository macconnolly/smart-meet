...
class BridgeDiscoveryRequest(BaseModel):
    ...
    novelty_weight: float = Field(0.6, ge=0.0, le=1.0, description="Weight for novelty in bridge scoring")
    connection_weight: float = Field(0.4, ge=0.0, le=1.0, description="Weight for connection potential in bridge scoring")
    min_bridge_score: float = Field(0.5, ge=0.0, le=1.0, description="Minimum score for a memory to be considered a bridge")
    max_bridges: int = Field(5, ge=1, le=20, description="Maximum number of bridges to return")
    search_expansion: int = Field(100, ge=10, le=500, description="Number of initial memories to consider for bridge discovery")

    class Config:
        schema_extra = {
            "example": {
                "query": "What are the unexpected connections between our marketing and engineering efforts?",
                "project_id": "proj-cross-functional",
                "max_bridges": 3
            }
        }
